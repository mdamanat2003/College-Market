...
import { Animated, Easing, Platform, View, Text, StyleSheet, ScrollView, TouchableOpacity, useWindowDimensions, Pressable } from 'react-native';
...
// 👇 Sub-component for Premium Hover/Tap Interactions 👇
function FeatureCardItem({ item, isTablet, isDesktop }: { item: any, isTablet: boolean, isDesktop: boolean }) {
  ...
  return (
    <Pressable
      ...
      style={({ pressed }) => [
        styles.gridFeatureCard,
        isTablet && styles.cardTablet,
        isDesktop && styles.cardDesktop,
        (isHovered || pressed) && styles.gridFeatureCardHovered,
      ]}
    >
      {/* 👇 CHILDREN KO FUNCTION ME WRAP KIYA HAI YAHAN 👇 */}
      {({ pressed }) => (
        <>
          {/* Subtle Neon Watermark Glow at Bottom Right */}
...
export default function LandingPage() {
  ...
  useEffect(() => {
    const gradientAnimation = Animated.loop(
      Animated.timing(welcomeGradient, {
        toValue: 1,
        duration: 9000,
  ...
  }, [welcomeGradient, textColorAnim]);

  const gradientLayerTwoOpacity = welcomeGradient.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [0, 1, 0],
  });
  const gradientLayerThreeOpacity = welcomeGradient.interpolate({
    inputRange: [0, 0.35, 0.75, 1],
    outputRange: [0.65, 0, 1, 0.65],
  });
  const gradientDrift = welcomeGradient.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [-18, 18, -18],
  });
  const welcomeTextScale = welcomeGradient.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [1, 1.045, 1],
  });
  const welcomeTextLift = welcomeGradient.interpolate({
    inputRange: [0, 0.5, 1],
    outputRange: [3, -3, 3],
  });
  ...
  return (
    <ScrollView ref={scrollRef} style={styles.page} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {/* 🌟 Fully Responsive Premium Bento Grid 🌟 */}
...
const styles = StyleSheet.create({
  ...
  welcomeGradientBase: {
    ...
    ...(Platform.OS === 'web' ? { backgroundImage: 'linear-gradient(120deg, #DBEAFE 0%, #CFFAFE 35%, #DCFCE7 68%, #FDE68A 100%)', backgroundSize: '180% 180%' } as any : null),
  },
  welcomeGradientLayerTwo: {
    backgroundColor: '#FBCFE8',
    ...(Platform.OS === 'web' ? { backgroundImage: 'linear-gradient(135deg, #FCE7F3 0%, #E0E7FF 42%, #BAE6FD 72%, #D9F99D 100%)', backgroundSize: '200% 200%' } as any : null),
  },
  welcomeGradientLayerThree: {
    backgroundColor: '#DDD6FE',
    ...(Platform.OS === 'web' ? { backgroundImage: 'linear-gradient(145deg, #FDE68A 0%, #A7F3D0 32%, #BFDBFE 66%, #F5D0FE 100%)', backgroundSize: '220% 220%' } as any : null),
  },
  welcomeSheen: {
    position: 'absolute',
  ...
  statCardMobile: {
    minWidth: 0, flexBasis: '48%', flexGrow: 1,
  },
  statValue: {
    color: '#fff',
    fontSize: 24,
  ...
  mockupWrap: {
    flex: 1, minWidth: 0, width: '100%', minHeight: 460, justifyContent: 'center',
  },
  mockupWrapMobile: { minHeight: 0 },
  mockupGlow: {
    position: 'absolute', left: 20, right: 20, top: 20, bottom: 20,
    borderRadius: 36, backgroundColor: 'rgba(59,130,246,0.16)',
  },
  mockupCard: {
    width: '100%', borderRadius: 28, backgroundColor: 'rgba(255,255,255,0.06)',
    borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', padding: 16,
    shadowColor: '#8b5cf6', shadowOpacity: 0.3, shadowRadius: 24, shadowOffset: { width: 0, height: 12 }, elevation: 12,
  },
  mockupTopBar: {
    flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 12,
    minHeight: 84, padding: 14, borderRadius: 26, borderWidth: 1, borderColor: 'rgba(255,255,255,0.1)', backgroundColor: 'rgba(0,0,0,0.38)',
  },
  ...
  onlinePill: {
    ...
    backgroundColor: 'rgba(16,185,129,0.1)', borderWidth: 1, borderColor: 'rgba(16,185,129,0.18)',
  },
  ...
  // Interactive Premium Feature Cards
  gridFeatureCard: {
    ...
    width: '100%', 
    minHeight: 220,
    overflow: 'hidden', // Contains watermark glow
    transitionProperty: 'all',
    transitionDuration: '300ms',
  },
  gridFeatureCardHovered: {

...
export default function Navbar() {
  // Ye state menu ko open/close track karegi
    ...
    <header className="sticky top-0 z-50 px-4 pt-4 relative">
      <nav className="mx-auto flex max-w-7xl items-center justify-between rounded-full border border-gray-200 bg-white/90 shadow-sm px-4 py-3 backdrop-blur-md md:px-6">
        ...
        <div className="hidden items-center gap-3 md:flex">
          <a href="http://localhost:8081/login" className="rounded-full border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-medium text-gray-700 transition hover:bg-gray-100">
            Sign In
          </a>
          <a href="http://localhost:8081/register" className="group rounded-full bg-blue-600 px-5 py-2 text-sm font-semibold text-white transition hover:bg-blue-700">
        ...
        >
          {/* Agar menu open hai toh 'X' (Close) icon dikhao, warna 'Menu' (Hamburger) icon dikhao */}
          {isMenuOpen ? <X className="h-5 w-5" /> : <Menu className="h-5 w-5" />}
        </button>
      </nav>

      {/* Mobile Menu Dropdown (Sirf tabhi dikhega jab isMenuOpen true hoga) */}
      {isMenuOpen && (
        <div className="absolute top-20 left-4 right-4 bg-white border border-gray-200 shadow-lg rounded-3xl p-6 md:hidden flex flex-col gap-4 z-50">
          ...
          <a 
            ...
            className="text-center rounded-full border border-gray-200 bg-gray-50 px-4 py-3 font-medium text-gray-700 transition hover:bg-gray-100"
          >
            Sign In
          </a>
          <a 
            href="http://localhost:8081/register" 
            className="text-center rounded-full bg-blue-600 px-4 py-3 font-semibold text-white transition hover:bg-blue-700"
          >
            Sign Up
          </a>
        </div>
